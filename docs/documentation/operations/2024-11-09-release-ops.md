...
## Mock data refresh
- Synced `services/nodejs-server/data/db.json`, `services/nodejs-server/data/workflows.json`, and `services/nodejs-server/data/workflowRuns.json` with the 2024-11-08 production snapshots.
...
- Rollback plan: restore `services/nodejs-server/data` from object storage snapshot `mockdata-2024-11-05`, reapply secrets from vault item `Workspace API Secrets — 2024-10-28`, redeploy container tag `workspace-api:2024-11-02.1`.
