...
## Summary
- Refreshed the local mock dataset under `services/nodejs-server/data` to mirror current production schema while maintaining anonymized identifiers.
...
- Validation checklist:
  - [x] `workspaceId` references map to defined workspaces in `services/nodejs-server/data.js`.
...
- **Rollback plan:**
  1. Restore `services/nodejs-server/data` from the `mock-dataset-backups/2024-05-12` snapshot in object storage.
