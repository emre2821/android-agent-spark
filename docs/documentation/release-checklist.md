...
- [ ] Frontend lint: `npm --prefix apps/web-app run lint`
- [ ] Frontend build & type check: `npm --prefix apps/web-app run build`
...
## Operations
- [ ] Confirm deployment manifests set `AGENT_SPARK_DB_PATH`, `AGENT_SPARK_API_KEY`, `AGENT_SPARK_DATA_DIR`, and `AGENT_SPARK_SCHEDULER_ENABLED` appropriately.【F:services/python-backend/app/config.py†L8-L28】【F:services/python-backend/app/api/dependencies.py†L18-L31】
- [ ] Back up or migrate the SQLite database (`data/agent_spark.db`) and any legacy vault files before rolling the release.【F:services/python-backend/app/config.py†L10-L28】【F:services/python-backend/app/db/legacy.py†L27-L67】
