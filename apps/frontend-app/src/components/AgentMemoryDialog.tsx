...
const TYPE_CLASSES: Record<MemoryType, string> = {
...
export const AgentMemoryDialog: React.FC<AgentMemoryDialogProps> = ({ open, onClose, agentId }) => {
  ...
  }, [agentId, fetchAgentMemory, open, resetEditor, resetState, toast]);
  ...
  const handleDeleteMemory = async (id: string) => {
    if (!agentId || !deleteMemoryItem) {
      toast({
        ...
        description: 'Memory service is unavailable.',
        variant: 'destructive',
      });
      return;
    }
      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh]">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Manage, add, edit, and delete memory items for the selected agent.
          </DialogDescription>
        </DialogHeader>
                ...
                {isLoading ? (
                  ...
                  memoryItems.map((item, index) => (
                    <React.Fragment key={item.id}>
                      <div className="space-y-4" data-testid={`memory-item-${index}`}>
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                          <div className="space-y-2 flex-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <h4 className="text-sm font-semibold text-foreground/90">{item.key}</h4>
                              <Badge variant="outline" className={TYPE_CLASSES[item.type]}>
                                {item.type}
                              </Badge>
                            </div>
                            {editingId === item.id ? (
