...
- If the dashboard immediately redirects back to `/login`, confirm `JWT_SECRET` is consistent between server restarts. Clearing `localStorage` and restarting the API usually resolves stale tokens.【F:nodejs-server/index.js†L1-L120】【F:src/hooks/use-auth.tsx†L1-L123】
...
- Syntax errors often stem from using ES module syntax without enabling "type": "module"—this repo already sets it in `package.json`. If you add new files under `nodejs-server/`, keep the `.js` extension and use `import`/`export` exclusively.
- Wrap asynchronous route handlers in `try/catch` to surface runtime errors cleanly instead of leaving the process hanging.
- Native dependencies such as `better-sqlite3` require build tooling. On macOS install Xcode Command Line Tools; on Linux make sure `build-essential` and `python3` are available before running `npm install`. If compilation still fails, set `AGENT_DB_PATH=:memory:` to unblock development while investigating.【F:nodejs-server/storage.js†L1-L120】
...
- The WebSocket endpoint lives at `/ws`. When deploying behind a proxy, forward `Upgrade` and `Connection` headers; otherwise connections will close immediately.【F:nodejs-server/index.js†L200-L320】
