...
function ensureWebBuild(projectRoot) {
  ...
  execSync('npm --prefix apps/web-app run build', {
    cwd: projectRoot,
    stdio: 'inherit',
  });
}
...
function stageDesktopBundle(projectRoot) {
  ...
  const webDist = path.join(projectRoot, 'apps', 'web-app', 'dist');
  const electronMain = path.join(projectRoot, 'platforms', 'electron-desktop', 'main.js');
  ...
  const buildInfo = {
    ...
    source: 'scripts/build/build_desktop.js',
